    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside id="sidebar" class="w-64 bg-slate-900 text-white flex flex-col fixed inset-y-0 left-0 z-50 transform -translate-x-full md:relative md:translate-x-0 transition duration-200 ease-in-out">
            <div class="p-6 flex items-center justify-center border-b border-slate-700">
                <span class="text-2xl font-bold font-sans">MBG Admin</span>
            </div>
            <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                <a href="{{ route('admin.dashboard') }}"
                    class="block px-4 py-2 rounded transition {{ request()->routeIs('admin.dashboard') ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-slate-800 hover:text-white' }}">
                    <i class="fas fa-home mr-2 w-5 text-center"></i> Dashboard
                </a>
                <a href="{{ route('admin.penerima.index') }}"
                    class="block px-4 py-2 rounded transition {{ request()->routeIs('admin.penerima.*') ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-slate-800 hover:text-white' }}">
                    <i class="fas fa-users mr-2 w-5 text-center"></i> Data Penerima
                </a>
                <a href="{{ route('admin.distribusi.index') }}"
                    class="block px-4 py-2 rounded transition {{ request()->routeIs('admin.distribusi.*') ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-slate-800 hover:text-white' }}">
                    <i class="fas fa-truck mr-2 w-5 text-center"></i> Titik Distribusi
                </a>
                <a href="{{ route('admin.status.index') }}"
                    class="block px-4 py-2 rounded transition {{ request()->routeIs('admin.status.*') ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-slate-800 hover:text-white' }}">
                    <i class="fas fa-map-marker-alt mr-2 w-5 text-center"></i> Status Pengiriman</a>
                <a href="{{ route('admin.laporan.index') }}"
                    class="block px-4 py-2 rounded transition {{ request()->routeIs('admin.laporan.*') ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-slate-800 hover:text-white' }}">
                ...
                <a href="{{ route('admin.pengguna.index') }}"
                    class="block px-4 py-2 rounded transition {{ request()->routeIs('admin.pengguna.*') ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-slate-800 hover:text-white' }}">
